...
<div class="admin-wrapper">
    ...
    <aside class="admin-sidebar">
        <div class="sidebar-header">
            <i class="bi bi-people-fill me-2"></i> {{ t('Daftar User') }}
        </div>
        <div class="p-3">
            <input type="text" id="searchUser" class="form-control mb-3" placeholder="{{ t('Cari User...') }}">
            <div class="user-list">
                <a href="{{ route('assessments.index') }}" class="user-item {{ request('user_id')==''?'active':'' }}">
                    <div class="fw-bold">
                        {{ t('Semua User') }}
                    </div>
                </a>
                @foreach($users as $user)
                <a href="{{ route('assessments.index', array_merge(request()->except('page'),['user_id'=>$user->id])) }}" class="user-item {{ request('user_id')==$user->id?'active':'' }}">
                    <div class="user-info">
                        <div class="user-name">
                            {{ $user->name }}
                        </div>
                    ...
                    <span class="badge bg-primary">{{ $user->assessments_count }}</span>
                </a>
                @endforeach
            </div>
        </div>
    ...
    <main class="admin-content">
        <div class="card report-card">
            <div class="card-header report-header d-flex justify-content-between align-items-center bg-primary text-white">
                <h5 class="mb-0 fw-bold"><i class="bi bi-clipboard-data me-2"></i> {{ t('Daftar Assessment') }}</h5>
            </div>
            ...
            <div class="card-body">
                        ...
                        <div class="card-body">
                            <form method="GET">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-2">
                                        <input type="date" name="assessment_date" value="{{ request('assessment_date') }}" class="form-control">
                                    </div>
                                    <div class="col-md-2">
                ...
                <table class="table list-table align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr onclick="window.location='{{ route('assessments.report', $assessment->id) }}'"
                        @endif
                        >
                        ...
                        <td class="progress-column">
                            ...
                            <div class="progress" style="height: 12px;">
                                ...
                                @else
                                bg-danger
                                @endif "role="progressbar" style="width: {{ $assessment->progress }}%;" aria-valuenow="{{ $assessment->progress }}" aria-valuemin="0" aria-valuemax="100"></div>
                            </div>
                        </td>
                        <td>
                            @if($assessment->status=='completed')
