            ...
            <div class="login-form">
                <div class="d-flex justify-content-end pe-1 mb-2">
                    <div class="btn-group btn-group-sm shadow-sm" role="group">
                        <div class="border bg-light d-flex align-items-center justify-content-center px-2" style="width:38px; cursor:default; user-select:none;">
                            <i class="bi bi-globe2 fs-5"></i>
                        </div>
                        <a href="{{ route('language.switch','id') }}" class="btn btn-sm px-2 {{ app()->getLocale()=='id' ? 'btn-success' : 'btn-outline-secondary' }}">
                            ID
                        </a>
                        <a href="{{ route('language.switch','en') }}" class="btn btn-sm px-2 {{ app()->getLocale()=='en' ? 'btn-success' : 'btn-outline-secondary' }}">
                            EN
                        </a>
                    </div>
                </div>
                ...
                <form action="{{ route('login') }}" method="POST">
                    ...
                    <div class="mb-4 position-relative">
                            <input id="password" class="form-control" type="password" name="password" placeholder="{{ t('Password') }}" required>
                            @error('password')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
